...
export default function Editor() {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: '#0d1117' }}>
      {/* Panel header + tabs */}
      <div
        className="flex items-end shrink-0 border-b border-[#30363d] overflow-x-auto"
          ...
          {filePaths.map((path) => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-1.5 px-3 shrink-0 border-r border-[#30363d] cursor-pointer"
                style={{
                  height: '36px',
...
function EditorIcon({ size = 14 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="currentColor" style={{ color: '#484f58' }}>
...
function FileIcon({ path }: { path: string }) {
  ...
  const color = ext === 'sql' ? '#ff694a' : ext === 'yml' || ext === 'yaml' ? '#3fb950' : '#7d8590'
  return (
